...
public class MultiPartMIMEReader {
    ...
    private class R2MultiPartMimeReader implements Reader {
        ...
        private final List<Byte> _byteArray = new ArrayList<Byte>();
        private int _currentPart = -1; //signals the need to read in the preamble
        ...
        //todo consider malformed bodies of all sorts!
        //todo max header limit
        @Override
        public void onDataAvailable(ByteString data) {

            while (_currentPart == -1) {
                //keep reading in until we have surpassed the premable and seen the first boundary
                //_buffer.write(data.copyBytes());
                appendByteStringToList(data);

                //todo improve this, this is n^2
                if (Collections.indexOfSubList(_byteArray, _boundaryBytes) > -1) {
                    _currentPart++;
                } else {
                    _rh.request(5);
                }
            ...
            //if we get here this means that we have gotten past the preamble
            //and we are on some part >= 0
            //keep reading in until we bring in all headers
