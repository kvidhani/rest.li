...
public class RestLiCallback<T> implements RequestExecutionCallback<T>
{
                        ...
                        final FilterRequestContext filterRequestContext)
  {
    ...
    _callback = callback;
    _responseFilterContextFactory = new RestLiResponseFilterContextFactory(_request, _routingResult, _responseHandler);
  ...
  private class RestLiResponseFilterChainCallbackImpl implements RestLiResponseFilterChainCallback
  {
    ...
    @Override
    public void onCompletion(final RestLiResponseData responseData, final RestLiStreamingAttachments streamingAttachments)
    {
      ...
      if (responseData.isErrorResponse())
      {
        ...
        // Note that any attachments provided by the resource in the response will not be sent back due to the error
      ...
      else
      {
        ...
        _callback.onSuccess(_responseHandler.buildResponse(_routingResult, response), _executionReport, streamingAttachments);
      }
    }
  }
}
